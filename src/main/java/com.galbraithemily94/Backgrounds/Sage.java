...
public class Sage extends Background {
    ...
    public String getEarlyLife(Character character) {
        ...
        earlyLives.add(sage1 + character.getSubjectPronoun() + " loved using " + character.getPossessivePronoun().toLowerCase() + " knowledge to try and create new potions. Eventually " + character.getSubjectPronoun().toLowerCase() + " became a well known alchemist whose potions were ordered and sought by all");
        earlyLives.add(sage1 + character.getSubjectPronoun() + " spent so much time in the library, that often times " + character.getSubjectPronoun().toLowerCase() + " would often end up falling asleep on a desk. People just began to assume " + character.getSubjectPronoun().toLowerCase() + " was a librarian, and so " + character.getSubjectPronoun().toLowerCase() + " became one");
